...
class Conversion:
    def __init__(self, df, sensorCode, sensorCodeWithChannel, eventTimestamp):
        ...
        getStats(self.df, 'detrended_velocity_ms')
        ...
        getStats(self.df, 'highpassed_displacement_cm')
    ...
    def plotGraph(self, df, column, plotTitle, padded=True):
        ...
        """
        if padded:
            plot = self.df.iloc[500:40500].reset_index().plot(kind='line', x='index', y=column, color='red', title=plotTitle)
        else:
